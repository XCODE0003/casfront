...
export default {
    ...
    data() {
        return {
            ...
            premioPositions: [
                0, 0.39, 0.79, 1.18, 1.57, 1.96, 2.35, 2.74, 3.14, 3.526, 3.92,
                4.32, 4.71, 5.1, 5.489999999999999, 5.880000000000001,
                6.280000000000001,
            ],
            positionsReverse: [],
            wheelPrizes: {},
    ...
    methods: {
        ...
        calculateTargetRotation(resultIndex) {
            const validIndex = Math.max(
                0,
                Math.min(resultIndex, this.premioPositions.length - 1)
            );
        ...
        createSectorText(sectorNum, icon) {
            ...
            const text = new PIXI.Text(
                Number(this.wheelPrizes[sectorNum].value)
                    .toFixed(2)
                    .replace(".", ","),
                {
                    fill: "#FFF",
        ...
        BuildWheel() {
            ...
            this.pixiLoader.load((loader, resources) => {
                vm.spinPoint = new PIXI.Sprite.from(
                    resources.spinPoint.texture
                );
                vm.buttonSprite = new PIXI.Sprite.from(
                    resources.spinBtn.texture
                );
                vm.circleSprite = new PIXI.Sprite.from(
                    resources.spinBackground.texture
                );
                vm.circleSpriteLight = new PIXI.Sprite.from(
                    resources.spinLight.texture
                );
                ...
                vm.centerContainer.fill = vm.centerContainer.x =
                    vm.app.screen.width / 2;
                ...
                const topTextAmount = new PIXI.Text("$100 mil", {
                    ...textOptions,
                    dropShadow: true,
                    dropShadowAlpha: 0.8,
                    dropShadowAngle: 1.5,
                ...
                const topImage = new PIXI.Sprite.from(
                    resources.spinHead.texture
                );
                ...
                vm.buttonSprite
                    ...
                    .on("pointertap", () => {
                        if (!vm.spinning) {
                            ...
                            vm.buttonSprite.scale.set(
                                vm.buttonSpriteScale - 0.04
                            );
                            setTimeout(() => {
                                gsap.to(vm.buttonSprite.scale, {
                                    x: vm.buttonSpriteScale,
